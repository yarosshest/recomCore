...
type ProductEdges struct {
	...
	// Vectors holds the value of the vectors edge.
...
func (e ProductEdges) AttributesOrErr() ([]*Attribute, error) {
	if e.loadedTypes[0] {
...
// was not loaded in eager-loading.
func (e ProductEdges) RatesOrErr() ([]*Rate, error) {
	if e.loadedTypes[1] {
		return e.Rates, nil
	}
...
// was not loaded in eager-loading.
func (e ProductEdges) VectorsOrErr() ([]*Vector, error) {
	if e.loadedTypes[2] {
...
func (*Product) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case product.FieldName, product.FieldPhoto, product.FieldDescription:
...
func (pr *Product) QueryAttributes() *AttributeQuery {
	return NewProductClient(pr.config).QueryAttributes(pr)
}

// QueryRates queries the "rates" edge of the Product entity.
func (pr *Product) QueryRates() *RateQuery {
	return NewProductClient(pr.config).QueryRates(pr)
}

// QueryVectors queries the "vectors" edge of the Product entity.
func (pr *Product) QueryVectors() *VectorQuery {
	return NewProductClient(pr.config).QueryVectors(pr)
}
