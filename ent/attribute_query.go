...
func (aq *AttributeQuery) QueryOwner() *ProductQuery {
...
func (aq *AttributeQuery) WithOwner(opts ...func(*ProductQuery)) *AttributeQuery {
	query := (&ProductClient{config: aq.config}).Query()
...
func (aq *AttributeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attribute, error) {
	...
	if query := aq.withOwner; query != nil {
		if err := aq.loadOwner(ctx, query, nodes, nil,
			func(n *Attribute, e *Product) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttributeQuery) loadOwner(ctx context.Context, query *ProductQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *Product)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Attribute)
	for i := range nodes {
		if nodes[i].product_attributes == nil {
			continue
		}
		fk := *nodes[i].product_attributes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_attributes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
