...
func (pq *ProductQuery) QueryAttributes() *AttributeQuery {
...
func (pq *ProductQuery) QueryRates() *RateQuery {
	query := (&RateClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) QueryVectors() *VectorQuery {
	query := (&VectorClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withRates:      pq.withRates.Clone(),
		withVectors:    pq.withVectors.Clone(),
...
func (pq *ProductQuery) WithAttributes(opts ...func(*AttributeQuery)) *ProductQuery {
	query := (&AttributeClient{config: pq.config}).Query()
...
func (pq *ProductQuery) WithRates(opts ...func(*RateQuery)) *ProductQuery {
	query := (&RateClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) WithVectors(opts ...func(*VectorQuery)) *ProductQuery {
	query := (&VectorClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withVectors = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withAttributes; query != nil {
		if err := pq.loadAttributes(ctx, query, nodes,
			func(n *Product) { n.Edges.Attributes = []*Attribute{} },
			func(n *Product, e *Attribute) { n.Edges.Attributes = append(n.Edges.Attributes, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withRates; query != nil {
		if err := pq.loadRates(ctx, query, nodes,
			func(n *Product) { n.Edges.Rates = []*Rate{} },
			func(n *Product, e *Rate) { n.Edges.Rates = append(n.Edges.Rates, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withVectors; query != nil {
		if err := pq.loadVectors(ctx, query, nodes,
			func(n *Product) { n.Edges.Vectors = []*Vector{} },
			func(n *Product, e *Vector) { n.Edges.Vectors = append(n.Edges.Vectors, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadAttributes(ctx context.Context, query *AttributeQuery, nodes []*Product, init func(*Product), assign func(*Product, *Attribute)) error {
	...
	for _, n := range neighbors {
		fk := n.product_attributes
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_attributes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_attributes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProductQuery) loadRates(ctx context.Context, query *RateQuery, nodes []*Product, init func(*Product), assign func(*Product, *Rate)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Rate(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.RatesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_rates
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_rates" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_rates" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProductQuery) loadVectors(ctx context.Context, query *VectorQuery, nodes []*Product, init func(*Product), assign func(*Product, *Vector)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Vector(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.VectorsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_vectors
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_vectors" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_vectors" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
